...
class BC3Parser:
    ...
    def _parse_D(self, data):
        """
        Parses a ~D record (Decomposition) according to FIEBDC-3 specification.
        
        Handles both formats:
        - Format 1 (Official): ~D | PARENT_CODE | CHILD_CODE\\FACTOR\\OUTPUT | CHILD_CODE\\FACTOR\\OUTPUT | ...
        - Format 2 (Packed): ~D | PARENT_CODE | CHILD_CODE\\FACTOR\\OUTPUT\\CHILD_CODE\\FACTOR\\OUTPUT\\... |
        
        Also handles percentage codes (% and & characters) and PERCENTAGE_CODE fields.
        """
        fields = self._split_fields(data)
        children = []
        
        if len(fields) < 2:
            # Invalid decomposition record
            return {
                "parent_code": fields[0] if len(fields) > 0 else '',
                "children": children,
                "parsing_error": "Missing decomposition data"
            }
        
        parent_code = fields[0]
        
        # Determine if we're dealing with packed format
        # Packed format: multiple children in one field separated by backslashes
        is_packed_format = False
        if len(fields) == 3 and fields[2].strip() == '':
            # Check if field[1] contains multiple triplets
            field_data = fields[1]
            if field_data.count('\\') >= 5:  # At least 2 complete child entries (each needs 3 parts minimum)
                is_packed_format = True
        
        if is_packed_format:
            # Format 2 (Packed): CHILD_CODE\FACTOR\OUTPUT\CHILD_CODE\FACTOR\OUTPUT\...
            self._parse_packed_decomposition(fields[1], children)
        else:
            # Format 1 (Official): Each field contains one child
            for field in fields[1:]:
                if not field.strip():
                    continue
                self._parse_single_decomposition_field(field, children)
    ...
    def _parse_single_decomposition_field(self, field, children):
        """Parse a single decomposition field: CHILD_CODE\\FACTOR\\OUTPUT[\\PERCENTAGE_CODES]"""
        if not field.strip():
            return
        ...
        is_percentage = '%' in child_code or '&' in child_code
        percentage_type = None
        if is_percentage:
            if '%' in child_code:
                percentage_type = 'non_cumulative'
            elif '&' in child_code:
                percentage_type = 'cumulative'
        
        # Parse factor (default = 1)
        factor = '1'
        if len(parts) > 1 and parts[1].strip():
            factor = parts[1].strip()
        
        # Parse output (default = 1) 
        output = '1'
        if len(parts) > 2 and parts[2].strip():
            output = parts[2].strip()
        
        # Parse percentage codes (semicolon-separated)
        percentage_codes = []
        if len(parts) > 3:
            for i in range(3, len(parts)):
                if parts[i].strip():
                    # Percentage codes are separated by semicolons
                    codes = [code.strip() for code in parts[i].split(';') if code.strip()]
                    percentage_codes.extend(codes)
        ...
        if is_percentage:
            child_entry["percentage_type"] = percentage_type
            
        if percentage_codes:
            child_entry["percentage_codes"] = percentage_codes
            
        children.append(child_entry)
    
    def _parse_packed_decomposition(self, packed_data, children):
        """Parse packed decomposition format: CHILD1\\FACTOR1\\OUTPUT1\\CHILD2\\FACTOR2\\OUTPUT2\\..."""
        if not packed_data.strip():
            return
        ...
        while i < len(parts):
            ...
            # Move to next child (skip the factor and output we just processed)
            i += 3
